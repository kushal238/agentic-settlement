...
## Parameter note: `3f` vs `3f+1`

Public FastSet documentation assumes **`3f+1`** validators with at most **`f`** Byzantine faults, and a quorum of **`2f+1`** signatures.

This project follows the course/plan parameterization: **`n = 3f`** validators and quorum **`2f+1`** valid certificates. The facilitator waits until **all `n`** validators have either responded or timed out before evaluating quorum.
...
sequenceDiagram
  ...
  participant V as Validators_n_eq_3f
