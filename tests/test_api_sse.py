"""Tests for the api_server's SSE event stream generator.

Mirrors tests/test_facilitator_sse.py. We test sse_event_stream directly
rather than going through the HTTP layer because httpx's ASGITransport
buffers the entire response before returning, which makes in-process
testing of infinite streaming responses impossible.
"""
...
from src.api_server.routes.events import sse_event_stream
...
async def test_stream_receives_events_from_threadsafe_publisher():
    """Even though api_server's /resource emits inline (no thread-safety
    needed), the bus must still support threadsafe publishers because the
    /events endpoint is shared infrastructure -- any future code that
    publishes from a worker thread should work."""
    bus = EventBus()
